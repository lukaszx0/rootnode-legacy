...
Readonly my $NGINX_VHOST_MAP   => '/etc/nginx/conf.d/map.conf';
Readonly my $NGINX_BLOCKED_MAP => '/etc/nginx/conf.d/blocked.conf'; 
...
sub new {
	...
	$db->{get_domain_list} = $dbh->prepare("SELECT uid, domain_name, active FROM domains");
...
sub reload_nginx {
	...
	# Prepare nginx map
	my (@nginx_vhost_map, @nginx_blocked_map);

	push @nginx_vhost_map, join("\n\t", 'map $http_host $ipaddr {', 'hostnames;', "default\t127.0.0.1;\n");
	push @nginx_blocked_map, join("\n\t", 'map $http_host $isblocked {', 'hostnames;', "default\t0;\n");

	while(my ($uid, $domain_name, $is_active) = $db->{get_domain_list}->fetchrow_array) {
		my $ipaddr = Satan::Tools->get_container_ip( $uid, $container_network );
		
		if ($is_active) {
			push @nginx_vhost_map, "\t".join("\t", ".$domain_name", $ipaddr).";\n";
		}
		else {
			push @nginx_blocked_map, "\t".join("\t", ".$domain_name", '1').";\n"; 
		}
	}
	push @nginx_vhost_map, "}\n";
	push @nginx_blocked_map, "}\n";

	# Save nginx map to file
	open my $vhost_fh, '>', $NGINX_VHOST_MAP or die "Cannot open file $NGINX_VHOST_MAP";
	print $vhost_fh @nginx_vhost_map;
	close $vhost_fh;

	open my $blocked_fh, '>', $NGINX_BLOCKED_MAP or die "Cannot open file $NGINX_BLOCKED_MAP";
	print $blocked_fh @nginx_blocked_map;
	close $blocked_fh;
